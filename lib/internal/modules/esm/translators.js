...
// Strategy for loading alt resource
translators.set('alt', async function moduleStrategy(url) {
  const _exports = process._linkedBinding('alt').getResourceExports(url.slice(4)/* slice alt: */);

  return createDynamicModule([], Object.keys(_exports), url, (reflect) => {
    debug(`Loading alt resource ${url}`);

    for (const key in _exports) {
      reflect.exports[key].set(_exports[key]);
    }
  });
});
